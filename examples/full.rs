...
mod views {
    ...
    // #[htmx_ssr(content="page", templates_root="full/", root=true)]
    // pub enum PageRoot {
    //    // Declare a page named `food` using `Food` as the model and `food_id` as the id.
    //    //
    //    // The `Option<PageFood>` is the optional sub-page and must point to another HtmxSsrView.
    //    #[htmx_ssr(name="food", id="food_id", id_type=usize, default)]
    //    Food(Food, Option<PageFood>),
    // }
    //
    // #[htmx_ssr(content="comment", templates_root="full/food/")]
    // pub enum PageFood {
    //    // Declare a page named `comment` using `Comment` as the model and `comment_id` as the id.
    //    #[htmx_ssr(name="comment", id="comment_id")]
    //    Comment(Comment),
    // }
